...
* Q: ファイルやフォルダの作成ができなくなりました。
  * もし、ターミナル上で `mkdir` や `cp` コマンドが使えるなら、以下の対処法で解決可能かと。
  * 対処1: browser を変えてみましょう。chrome をつかっていたのを、Microsoft Edge に変えて解決したケースがあります。おそらく、シークレットウィンドウやプライベートウィンドウを使うのでも解決するんじゃないかと。
  * 対処2: ターミナル上で、`mkdir` コマンドや `cp` コマンドでファイルやフォルダを作ってください。あとは、Explorer 経由で操作・編集可能かと。コピー操作は、Explorer 画面上の右クリック操作で、Duplicate してから Rename するのでもよいかと。
